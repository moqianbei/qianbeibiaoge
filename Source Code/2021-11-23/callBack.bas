...
Sub premierLevel(control As IRibbonControl)
    Dim selsht As Worksheet, rng As Range
    ...
    Else
        ...
        '隐藏sheet，不在前端显示
        ActiveWorkbook.Worksheets("中国所有区划名称表（勿删）").Visible = False
        ...
        Selection.Cells(1).Offset(0, 1) = "=INDIRECT(" & Selection.Cells(1).Address(0, 0) & ")"
...
'需要新建表格并复制表头，且空出第一列以放置各sheet名称
    ...
    Else
        Application.ScreenUpdating = False
        Dim arr() As String '选中的工作表名称
        Dim i As Long, con As Long
        ...
        Else
            ...
            Dim arow, acol  '数据起始单元格行号列号
            arow = rng.Row
            acol = rng.Column
            Dim newsht As Worksheet

            Set newsht = Sheets.Add(before:=Sheets(1), Count:=1) '需添加count，因为默认会添加你选择sheet的数量
            ...
            For i = LBound(arr) To UBound(arr)
                With Sheets(arr(i))
                    ...
                    If i = LBound(arr) Then
                        Range("B2").Select
                        ActiveSheet.Paste
                        newsht.Range(newsht.[A2], newsht.[B2].End(xlDown).Offset(0, -1)) = .Name
                    Else
                        Range("B2").End(xlDown).Offset(1, 0).Select
                        ActiveSheet.Paste
                        newsht.Range(newsht.[A2].End(xlDown).Offset(1, 0), newsht.[B2].End(xlDown).Offset(0, -1)) = .Name
                    End If
                End With
            Next
